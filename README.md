...
- `d1/seed.sql` inserts 12 researched opportunities. Live boards show more (e.g. 40) because the research agent appends proposals every 6h and humans add rows. Seed runs once on an empty table; live count = 12 seeds + agent proposals + manual adds. Killed rows stay listed, so the count only grows. The seed file is re-runnable (every statement is a no-op when its row already exists) and the deploy.sh seed guard aborts the deploy instead of seeding when it cannot measure the table. Live tables created before 2026-09-20 may lack the money columns (`revenue_cents`, `spent_cents`); `/api/health` then names them in `schema_missing_columns` with a disabled line until the owner sets `ALLOW_SCHEMA_MIGRATION=1` (Pages env var), after which the next health check adds exactly the two `ADD COLUMN` statements from `d1/migrate-2026-09-20-experiment-cents.sql` (once per isolate, no endpoint) and reports numbers on the same call.
