...
- A failed API fetch shows a named error banner with Retry (and `db down` turns the agent pill red) instead of an empty board (the review tab keeps its stale list and shows the same failure instead of the vetted-or-killed all-clear); `testing · 0 experiments` and `running exp · opp still researching` badges flag status mismatches read-only — the human still owns every status move.
