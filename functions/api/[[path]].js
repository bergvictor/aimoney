...
export async function onRequest(context) {
  ...
  try {
    if (parts.length === 1 && parts[0] === "health" && method === "GET") {
      ...
      // Failure honesty (audit 2026-09-20-round2 Task 4): a failed status probe
      // reads null (unknown), never {} (no experiments) — mirroring the
      // neighboring counts. Key order preserved for verify.sh.
      // Inflow-paused bit (audit 2026-09-20-round4 F4): derived from the
      // existing unreviewed probe — no new query. Mirrors the worker pause
      // gate (UNREVIEWED_BACKLOG_CAP = 10, worker/src/index.js:18): >10
      // pauses inflow. Null on outage/probe failure like its neighbors.
      const inflow_paused = (healthDown || probeFailed(1)) ? null : ((unreviewedRow ? unreviewedRow.n : 0) > 10);
